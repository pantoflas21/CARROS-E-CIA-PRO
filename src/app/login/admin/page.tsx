...
import { User, Info, ChevronRight, AlertCircle, Shield } from 'lucide-react';
...
function AdminLoginForm() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!checkRateLimit(`admin-login-${sanitizedEmail}`, 5, 60000)) {
      ...
      <div className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl rounded-2xl shadow-2xl p-8 max-w-md w-full relative z-10 border border-white/20">
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          ...
          <Button
            ...
            isLoading={loading}
            disabled={loading}
            className="w-full text-base py-3 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700"
          >
        ...
        <div className="mt-6 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
          ...
          <div className="space-y-1 text-xs text-blue-800 dark:text-blue-200 font-mono">
            <p>Email: <span className="font-semibold">admin@kinito.com</span></p>
            <p>Senha: <span className="font-semibold">Admin@123</span></p>
          </div>
        </div>
      </div>
    </div>
