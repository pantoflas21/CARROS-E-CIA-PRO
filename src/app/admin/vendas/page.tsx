...
function VendasContent() {
  ...
  useEffect(() => {
    ...
    if (success) {
      // Mostrar notificação de sucesso (pode implementar um toast)
      setTimeout(() => {
  ...
  const loadSales = async () => {
    try {
      ...
      const totalVendas = salesWithRelations.length;
      const totalVendido = salesWithRelations
        .filter(s => s.status === 'vendido')
        .reduce((sum, s) => sum + parseFloat(String(s.valor_venda)), 0);
      const totalComissoes = salesWithRelations
        .filter(s => s.status === 'vendido')
    ...
    <DashboardLayout role="admin">
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-400 mt-1">
              Gestão completa de vendas do sistema
            </p>
          </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>
              Todas as Vendas ({filteredSales.length})
            </CardTitle>
          </CardHeader>
          <CardContent>
